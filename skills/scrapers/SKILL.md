...
Superseding plus re-materializing was not sufficient, because the stale short was the cause, and superseding the observations does not remove it: `shortDescription` is not in `CLEARABLE_ON_EMPTY_RESEARCH_ENTITY_FIELDS`, so the projected card outlives its observation and the record stays blank across any number of passes until `shortDescription` and `fieldProvenance.shortDescription` are unset on the entity document too.
Do not assume the prior pair is safe to restore. The `studentReadyDescription` emit block in `server/src/scrapers/sources/labMicrositeUndergradLLMExtractor.ts` emits one string as `fullDescription` at 0.55 and the same string again as `shortDescription` when it is card-length.
Attribution, not duplication, decides whether that pair is stable: both pushes share one `...base`, so the two rows carry the same `sourceName` and `sourceUrl`, the materializer reads the projected short as self-derived from the full, the guard is skipped, and the row keeps serving.
The same string attributed across two URLs or two sources, which is what a hand repair and any second source produce, reads as independent evidence and blanks the full, so run `describeDescriptionPairRisk` on the pair as it will be attributed and fix the emitting source before repairing any row.
Build the query with `server/src/scripts/descriptionPairRollbackCore.ts` so it cannot be scoped to one field or to one identity form by accident, verify on the served record with `describeDescriptionPairRisk` rather than on the supersede count, passing the whole document so the short goes through the guard's own self-derived exclusion, and include an empty-`fullDescription`-on-`student_ready` line in any post-run diff.
