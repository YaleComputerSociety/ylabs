...
- `utils/hostConcurrencyLimiter.ts` - the shared per-host concurrency substrate for both fetch paths (the axios interceptor / `renderedFetch` sweep path, and the `httpFetch` `HostRateLimiter`, which delegates its per-host slot and spacing to this class), plus the `HOST_THROTTLE_OVERRIDES` map. `medicine.yale.edu` and `ysph.yale.edu` are empirically rate/concurrency throttled, not IP-banned (a 403 storm at high concurrency recovers instantly on the next single request; a ban would persist), so those hosts are pinned to concurrency 2 with a 400ms minimum inter-request interval (proven-safe: 60 profiles at that setting returned 60/60 200, zero 403). Add a further rate-limited host by adding one entry to `HOST_THROTTLE_OVERRIDES`; every other host keeps `DEFAULT_PER_HOST_CONCURRENCY` and no spacing. This unblocks direct YSM scraping (`ysm-faculty-directory` and the medicine-hosted lab-microsite fetches); dropping 403 from the retryable set for these hosts is a reasonable follow-up since retrying a throttle-403 without slowing is wasteful.
