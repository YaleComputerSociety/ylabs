...
- `canonicalResearchHomeResolver.ts` - lets NIH and NSF grant sources enrich one existing official research home for a matched PI. It permits a synthetic shell only when the PI has no research-home memberships and fails closed for ambiguous, archived, or grant-only candidates.
