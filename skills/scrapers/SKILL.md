...
- `canonicalResearchHomeResolver.ts` - lets NIH and NSF grant sources enrich one existing official research home for a matched PI. It permits a synthetic shell only when the PI identity is absent and has no research-home memberships, and fails closed for ambiguous identities plus archived, non-current, or grant-only candidates.
