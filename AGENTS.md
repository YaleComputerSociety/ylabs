...
The authenticated Fellowships page derives program state through `client/src/utils/fellowshipStatus.ts` from `isAcceptingApplications`, `applicationOpenDate`, `deadline`, and eligibility metadata. Browse cards/list rows, the detail modal, and admin edit surfaces show consistent status labels, deadline details, opening-soon/closed states, and eligibility warnings; the page groups opportunities into Closing Soon, Open, Opening Soon, and Closed sections.
...
Focused component and utility tests cover behavior outside the reducer pattern: `client/src/components/shared/__tests__/BrowseGrid.a11y.test.tsx` verifies keyboard-openable browse cards/list rows, separate favorite controls, and fellowship status badges; `client/src/components/shared/__tests__/ListingDetailModal.public.test.tsx` covers public listing dialog naming, Escape close behavior, focus trapping/restoration, and redacted contact actions; `client/src/components/accounts/ListingForm/FormFields/__tests__/ResearchAreaInput.a11y.test.tsx` covers listing-form research-area field selector dialog focus behavior; `client/src/components/fellowship/__tests__/FellowshipModal.test.tsx`, `client/src/pages/__tests__/fellowships.grouping.test.tsx`, `client/src/types/__tests__/browsable.test.ts`, and `client/src/utils/__tests__/fellowshipStatus.test.ts` cover fellowship timing, eligibility, grouping, and shared browse metadata.
