...
const ConfigContextProvider: FC<ConfigContextProviderProps> = ({ children }) => {
  ...
  const getResearchAreaByName = useCallback(
    (name: string): ResearchAreaConfig | undefined => {
      return researchAreaMap.get(name.toLowerCase());
    },
    [researchAreaMap],
  );

  const getColorForResearchArea = useCallback(
    (name: string) => {
  ...
  const getDepartmentByName = useCallback(
    (name: string): DepartmentConfig | undefined => {
  ...
  const getDepartmentsByCategory = useCallback(
    (category: string): DepartmentConfig[] => {
