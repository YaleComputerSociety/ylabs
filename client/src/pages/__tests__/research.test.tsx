...
describe('Research page', () => {
  it('renders the search-first shell with recognition-first quick starts', async () => {
    ...
    expect(container.textContent).toContain(
      'Search by interest, professor, course topic, or question.',
    );
    ...
    expect(
      screen.getByPlaceholderText('Type a topic, professor, lab, or research question'),
    ).toBeTruthy();
  ...
  it('shortens the hero search placeholder on compact viewports so it does not clip mid-word', async () => {
  ...
  it('collapses the result count to a lightweight total-first status line without a banner heading', async () => {
    mockSearchResponses((url) =>
      url === '/research/search'
        ? researchSearchResponse([researchEntity], { estimatedTotalHits: 29 })
        : unexpectedSearchEndpoint(url),
    );
