...
const Account = () => {
  ...
  const updateFellowshipFavorite = (fellowshipId: string, favorite: boolean) => {
    ...
    if (favorite) {
      ...
      setFavFellowships(
        prevFavFellowships.filter((f) => (f.id || (f as any)._id) !== fellowshipId),
      );
  ...
  const updateListing = (listing: Listing) => {
    setOwnListings((prevOwnListings) =>
      prevOwnListings.map((l) => (l.id === listing.id ? listing : l)),
    );
    setFavListings((prevFavListings) =>
      prevFavListings.map((l) => (l.id === listing.id ? listing : l)),
    );
  };
  ...
  const clearCreatedListing = () => {
    setOwnListings((prevOwnListings) =>
      prevOwnListings.filter((listing) => listing.id !== 'create'),
    );
    setIsEditing(false);
    setIsCreating(false);
  };
            ...
            <div className="bg-amber-100 border-l-4 border-amber-500 text-amber-700 p-4 mb-6 rounded shadow-sm">
              <div className="flex items-center">
                <p className="font-medium">
                  ...
                  publicly visible as favorites or in search results until your account is
                  confirmed.
                </p>
              </div>
            </div>
