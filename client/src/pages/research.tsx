...
const Research = () => {
  ...
  const [isCompactSearchViewport, setIsCompactSearchViewport] = useState(
    () => window.matchMedia?.('(max-width: 639px)').matches ?? false,
  );
  useEffect(() => {
    const mediaQuery = window.matchMedia?.('(max-width: 639px)');
    if (!mediaQuery) return;
    const handleChange = (event: MediaQueryListEvent) => setIsCompactSearchViewport(event.matches);
    setIsCompactSearchViewport(mediaQuery.matches);
    mediaQuery.addEventListener?.('change', handleChange);
    return () => mediaQuery.removeEventListener?.('change', handleChange);
  }, []);
  const searchPlaceholder = isCompactSearchViewport
