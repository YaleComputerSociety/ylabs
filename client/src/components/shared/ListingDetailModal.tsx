          ...
          <div className="px-6 py-5">
            <div className="flex items-start justify-between gap-4">
                    ...
                    <span className="text-sm font-semibold text-blue-700">
                      {(() => {
                        ...
                        if (primary && deps.length > 1) {
                          const idx = deps.findIndex(d => d === primary || getDepartmentAbbreviation(d) === getDepartmentAbbreviation(primary));
                          if (idx > 0) {
                            deps.splice(idx, 1);
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                  ...
                  <>
                    {listing.websites && listing.websites.length > 0 && (
                      <a href={ensureHttpPrefix(listing.websites[0])} onClick={(e) => e.stopPropagation()} target="_blank" rel="noopener noreferrer"
                        className="p-2 rounded-lg hover:bg-gray-100 transition-colors text-gray-500 hover:text-blue-600" title="Visit website">
                    ...
                    <a href={`mailto:${listing.ownerEmail}`} onClick={(e) => e.stopPropagation()}
                      className="p-2 rounded-lg hover:bg-gray-100 transition-colors text-gray-500 hover:text-blue-600" title="Send email">
                      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                ...
                <button onClick={onClose} className="p-2 rounded-lg hover:bg-gray-100 transition-colors text-gray-400 hover:text-gray-600" aria-label="Close">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div className="col-span-1 space-y-6">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {researchAreas.map((area: string) => {
                        ...
                        if (onNavigateToResearchArea) {
                          return (
                            <button key={area} onClick={() => onNavigateToResearchArea(area)} className={`${colors.bg} ${colors.text} text-xs rounded-md px-2 py-1 hover:ring-2 hover:ring-offset-1 cursor-pointer transition-all`}>
                              {area}
                            </button>
                          );
                        }
                        return (
                          <span key={area} className={`${colors.bg} ${colors.text} text-xs rounded-md px-2 py-1`}>
                            {area}
                          </span>
                        );
                      })}
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {(() => {
                        ...
                        if (onNavigateToDepartment) {
                          return (
                            <button key={dept} onClick={() => onNavigateToDepartment(fullName)} className="bg-gray-100 text-gray-700 text-xs rounded-md px-2 py-1 hover:bg-gray-200 hover:ring-2 hover:ring-gray-300 cursor-pointer transition-all">
                              {fullName}
                            </button>
                          );
                        }
                        return (
                          <span key={dept} className="bg-gray-100 text-gray-700 text-xs rounded-md px-2 py-1">
                            {fullName}
                          </span>
                        );
                      })}
                ...
                <section>
                  ...
                  <div className="space-y-2">
                    {[listing.ownerEmail, ...listing.emails].map((email, i) => (
                      <a key={i} href={`mailto:${email}`} className="flex items-center gap-2 text-sm text-blue-600 hover:text-blue-800 hover:underline">
                    ...
                    {listing.websites && listing.websites.length > 0 && listing.websites.map((website, i) => (
                      <a key={i} href={ensureHttpPrefix(website)} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-blue-600 hover:text-blue-800 hover:underline">
                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="flex-shrink-0">
                ...
                <section>
                  ...
                  <div className="space-y-1.5 text-sm">
                    {!restrictedStats && (
                      <>
                        <div className="flex justify-between"><span className="text-gray-500">Views</span><span className="font-medium text-gray-800">{listing.views}</span></div>
                        <div className="flex justify-between"><span className="text-gray-500">Favorites</span><span className="font-medium text-gray-800">{listing.favorites}</span></div>
                      </>
                    )}
                    {listing.established && (
                      <div className="flex justify-between"><span className="text-gray-500">Established</span><span className="font-medium text-gray-800">{listing.established}</span></div>
                    )}
                    <div className="flex justify-between"><span className="text-gray-500">Added</span><span className="font-medium text-gray-800">{new Date(listing.createdAt).toLocaleDateString()}</span></div>
                  </div>
                </section>
              </div>
              ...
              <div className="col-span-1 md:col-span-2 space-y-6">
                <section>
                  ...
                  <div className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">{listing.description}</div>
                </section>
                {listing.applicantDescription && listing.applicantDescription.trim() !== '' && (
                  <section>
                    <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Applicant Prerequisites</h3>
                    <div className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap bg-amber-50/50 border border-amber-100 rounded-lg p-4">{listing.applicantDescription}</div>
                  </section>
                )}
                {listing.archived && (
                  <div className="p-4 bg-red-50 border border-red-200 text-red-700 rounded-lg">
                    <div className="font-semibold text-sm">This listing is archived</div>
                    <div className="text-sm mt-1">Archived listings are not visible in search results or as favorites.</div>
                  </div>
                )}
              </div>
            </div>
