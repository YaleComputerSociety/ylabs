...
const BrowseCard = React.memo(
  ({ item, isFavorite, onToggleFavorite, onOpenModal, onAdminEdit }: BrowseCardProps) => {
    ...
    const itemLabel = isListing
      ? `View ${professorName} research listing, ${item.data.title}`
      : `View fellowship, ${item.data.title}`;

    return (
      <div
        className={`group relative bg-white rounded-lg border ${isAudited ? 'border-green-400 ring-1 ring-green-200' : 'border-gray-200'} hover:border-blue-400 hover:shadow-md transition-all duration-200 overflow-hidden h-full flex flex-col`}
      >
        <button
          type="button"
          className="h-full w-full flex flex-col text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:ring-offset-2"
          onClick={handleClick}
        ...
        >
          ...
          <div className="p-5 pr-16 flex-1 flex flex-col">
            {isListing ? (
              <>
                {deptLabel && (
                  <p className="text-sm font-semibold text-blue-700 mb-1 truncate">{deptLabel}</p>
                )}
                ...
                {tags.length > 0 && <div className="border-t border-gray-100 my-2" />}
