  ...
  const renderItems = () => {
    ...
    if (permanentValue) {
      elements.push(
        ...
        >
          <span className="whitespace-nowrap">{permanentValue}</span>
          <div
  ...
  return (
    <div className="mb-4">
      ...
      {(permanentValue || items.length > 0) && (
        <div className="flex flex-wrap gap-2 mt-2 overflow-x-auto">{renderItems()}</div>
      )}
      {infoText && <div className="text-xs text-gray-500 mt-1">{infoText}</div>}
