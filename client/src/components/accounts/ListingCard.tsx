...
const ListingCard = ({
  ...
  const handleArchive = (e: React.MouseEvent) => {
    ...
    if (archived) {
      setArchived(false);
      axios
        .put(`/listings/${listing.id}/unarchive`, { withCredentials: true })
        .then((response) => {
      ...
      setArchived(true);
      axios
        .put(`/listings/${listing.id}/archive`, { withCredentials: true })
        .then((response) => {
          const responseListing = response.data.listing;
          const listing = createListing(responseListing);
          updateListing(listing);
  ...
  return (
    <div className="mb-4 relative">
      <div key={listing.id} className="flex relative z-10 rounded-md shadow">
        <div
          className="group/card p-4 flex-grow grid grid-cols-3 md:grid-cols-12 cursor-pointer bg-white hover:bg-gray-50 border border-gray-300 hover:border-[#257fce] rounded-md transition-colors"
        ...
        >
          <div className="col-span-2 md:col-span-4">
            ...
            >
              {listing.departments && listing.departments.length > 0
                ? listing.departments
                    .slice(0, 3)
                    .map((dept) => getDepartmentAbbreviation(dept))
                    .join(' | ')
                : '\u00A0'}
            </span>
            <p
              className={`text-lg font-semibold mb-3 ${archived ? 'opacity-50' : ''}`}
              style={{
                lineHeight: '1.2rem',
                height: '1.2rem',
                overflow: 'hidden',
            ...
            <p
              className={`text-sm text-gray-700 ${archived ? 'opacity-50' : ''}`}
              style={{
                overflow: 'hidden',
                display: '-webkit-box',
                WebkitLineClamp: 1,
            ...
            >
              ...
              {[
                ...
                ...listing.professorNames,
              ].join(', ')}
            </p>
            <div
              ref={researchAreasContainerRef}
          ...
          <div className="col-span-6 hidden md:flex align-middle">
            <div
              className={`flex-shrink-0 border-l border-gray-300 mx-4 ${archived ? 'opacity-50' : ''}`}
            />
            <p
              className={`flex-grow text-gray-800 text-sm ${archived ? 'opacity-50' : ''}`}
              style={{
                display: '-webkit-box',
                WebkitLineClamp: 4,
                WebkitBoxOrient: 'vertical',
          ...
          <div className="flex flex-col col-span-1 md:col-span-2 items-end">
            <div className="flex items-start">
              ...
              <span
                className={`text-xs px-2 py-1 rounded ${archived ? 'opacity-50' : ''} ${
                  isOpen ? 'bg-green-500/20 text-green-700' : 'bg-red-500/20 text-red-700'
                }`}
              >
                  ...
                  <a onClick={toggleFavorite} className="inline-block relative group">
                    ...
                    >
                      <svg
                        ...
                        onMouseEnter={(e) => {
                          e.currentTarget.style.stroke = '#0055A4';
                          if (!isFavorite) e.currentTarget.style.fill = 'none';
                        }}
                        onMouseLeave={(e) => {
                          e.currentTarget.style.stroke = isFavorite ? '#0055A4' : '#5B646F';
                          e.currentTarget.style.fill = isFavorite ? '#0055A4' : 'none';
                        }}
                      >
                  ...
                  <a
                    href={
                      listing.websites && listing.websites.length > 0
                        ? ensureHttpPrefix(listing.websites[0])
                        : undefined
                    }
                    className={
                      listing.websites && listing.websites.length > 0
            ...
            <p
              className={`text-[8px] mb-0.5 text-gray-700`}
              style={{
                overflow: 'hidden',
                textOverflow: 'ellipsis',
                whiteSpace: 'nowrap',
      ...
      <div
        className={`transform transition-all duration-700 overflow-hidden ${editable && editing ? 'translate-y-0 max-h-[4000px]' : '-translate-y-5 max-h-0'} pl-2 pr-0.5 -mt-1`}
      >
        {editable && editing && (
          <ListingForm
            listing={listing}
