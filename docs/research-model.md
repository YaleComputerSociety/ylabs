...
When a link is proved gone the lane probes replacement candidates in trust order (an observed same-slug URL, then an observed same-host page whose slug names the same person, then the `/profile/<slug>` twin of the dead path, then `/profile/` and `/people/` pages named after the person rather than after the dead slug) and adopts one only if it too probes live, which is how a department re-slug (`/people/douglas-stone` becoming `/profile/a-douglas-stone`) is followed.
The name-derived candidates carry the cases the dead slug cannot reach (#2307): a stored slug that never named the person (`politicalscience.yale.edu/ian-home`), and a department that moved someone the opposite way to the usual migration (`ysph.yale.edu` moved a professor from `/profile/<slug>` to `/people/<slug>`).
Matching a slug to a person needs surname equality plus a given-name token that agrees whole or as a short form, so a nickname page (`philip-gorski` published as `phil-gorski`) is followed while an initial is still refused: `a` must never stand in for `alison`, because that is how a same-surname colleague gets claimed (#2308, #468).
Person pages are read from `/people/<slug>`, `/profile/<slug>`, `/person/<slug>`, and section-nested `/people/<section>/<slug>` (#2310).
