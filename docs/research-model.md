...
For-credit, course-based research pathways (directed research, independent study, and senior-essay/senior-thesis course sequences) are acquired as `COURSE_SEQUENCE` research homes by `courseBasedResearchPathwayScraper.ts`, which cites each department's own directed-research/senior-thesis course page (never a catalog or course-search index root) and is discovery-only: it emits identity, the official page URL, and a source-backed description, and fails closed on contact data.
...
Under the organizational/program dead-end gate (issue #1359), a lead-exempt entity with no attached lead and no reachable alternate access path (a linked related entity or a discovered people/get-involved/programs page) is held at `operator_review` with `missing_alternate_access_path` rather than auto-published; a pilot `COURSE_SEQUENCE` whose only evidence is its own course page is therefore correctly held for review, not silently trusted.
`COURSE_SEQUENCE` is deliberately not in `IDENTIFIED_LEAD_WAYS_IN_ENTITY_TYPES` in `accessMaterializer.ts`; wiring a course-sequence access ways-in (so a well-described pathway can reach `student_ready`) is a follow-up, tracked separately from the pilot acquisition path.
