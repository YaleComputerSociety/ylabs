...
The arbiter there is the entity's identity, never `confidence` (anti-correlated on both rows with ground truth: 0.9 on a wrong lead against 0.78 on the right one) and never `rosterProvenance` presence (absent on 974 of 3,009 served sole PIs who are correct), and an entity whose identity names two of the clashing people is a name-variant merge rather than a graft, so it is left alone (#2768).
