...
For-credit, course-based research pathways (directed research, independent study, and senior-essay/senior-thesis course sequences) are acquired as `COURSE_SEQUENCE` research homes by `courseBasedResearchPathwayScraper.ts`, which cites each department's own directed-research/senior-thesis course page (never a catalog or course-search index root) and emits identity, the official page URL, a source-backed description, and the for-credit access fact the cited page establishes (`offersIndependentStudy`).
A course sequence is treated as a program-style, lead-exempt pathway in the student-visibility gate: its "lead" is the DUS or course instructor and the pathway itself (enroll in the course, contact the department) is the way in, so a lead-less `COURSE_SEQUENCE` is never floored with `missing_lead`.
The `offersIndependentStudy` observation is derived by `accessMaterializer.ts` into a `CREDIT_FORMALIZATION_POSSIBLE` course-credit ways-in carrying the official page as its source, which is the concrete next step a student takes, so a well-described pathway reaches `student_ready`; the scraper still fails closed on contact data and never emits posted openings, application links, or contact routes.
Under the organizational/program dead-end gate (issue #1359), a lead-exempt entity with no attached lead and no reachable alternate access path (a linked related entity or a discovered people/get-involved/programs/undergraduate-research/directed-research page) is held at `operator_review` with `missing_alternate_access_path` rather than auto-published; a `COURSE_SEQUENCE` whose department page is such an engagement page clears this, while one whose only evidence is an unrecognized landing page is still held for review.
