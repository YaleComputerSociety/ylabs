...
- A constant contact prompt that always offers a way to reach out - opening the official profile or an available official source page, with the Yale Directory as a last resort when no official link exists, to find contact details and introduce yourself, with a prefilled mailto only when a non-redacted email exists - shown alongside source-backed signals, rather than a computed verdict or ranked next-step CTA.
