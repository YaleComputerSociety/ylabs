...
- [`client/src/App.tsx`](../client/src/App.tsx): routes `/` through the authenticated root redirect; live public app surfaces are `/programs`, `/research`, `/research/:slug`, `/opportunities/:id`, and `/account`. Retired client routes `/pathways` and `/listings` redirect to `/research`; `/fellowships` redirects to `/programs`.
- [`client/src/components/navigationLinks.ts`](../client/src/components/navigationLinks.ts): primary navigation links are Yale Research, Programs & Fellowships, and Dashboard.
- [`client/src/components/Navbar.tsx`](../client/src/components/Navbar.tsx): primary navigation, search, filters, and account controls for the live surfaces.
...
- [`client/src/pages/fellowships.tsx`](../client/src/pages/fellowships.tsx): `/programs` page for structured programs and fellowships.
- [`client/src/pages/opportunityDetail.tsx`](../client/src/pages/opportunityDetail.tsx): `/opportunities/:id` detail page for real posted opportunities.
- [`client/src/components/shared/BrowseCard.tsx`](../client/src/components/shared/BrowseCard.tsx): shared card treatment still used by legacy listing/program contexts and research groups.
...
Backend route note: `POST /api/pathways/search` remains Ways In infrastructure for embedded evidence and future filters. `/api/fellowships` remains a deprecated compatibility API with `/api/programs` as its successor.
...
### Ways In On `/research` And `/research/:slug`

Purpose: practical evidence for how a student might enter research, embedded in the live Research surfaces rather than exposed as a standalone public route.

The experience should answer: "Given my constraints, what routes can I act on?"

Primary UX ingredients:

- Filter and summarize by route type, compensation/funding possibility, evidence strength, active posted role, and best next step.
- Research cards and detail sections that show route label, host research entity, evidence strength, relevant evidence snippets, and the next action.
...
Current gap: Ways In evidence is present but still partly mediated through older access-verdict labels. The next pass should make Best Next Step and Evidence feel like the organizing structure without reviving `/pathways` as a public client page.

### Retired Client Redirects

Purpose: preserve old direct links without keeping legacy surfaces in the public IA.

- `/pathways` redirects to `/research`; Ways In infrastructure remains backend-only through `POST /api/pathways/search`.
- `/listings` redirects to `/research`; real active posted roles should resolve through `/opportunities/:id` and embedded Research context.
- `/fellowships` redirects to `/programs`; `/api/fellowships` remains deprecated compatibility with `/api/programs` as successor.

Do not reintroduce these as primary navigation items. Historical notes can mention them when dated, but current UX direction should treat `/research`, `/programs`, `/research/:slug`, `/opportunities/:id`, and `/account` as the live client contract.
...
- A deduped Sources section that shows each official source once and explains which Ways In evidence/routes it supports.
...
Current gap: the detail page now presents Ways In evidence, best next step, and deduped sources before active opportunities. The next improvement should reduce duplication between the right-rail route CTA and the main Best Next Step CTA without hiding the action on mobile.
...
2. Make Research cards and detail sections more action-oriented: route, entity, evidence, next step, and guarded CTA should be visually obvious.
3. Keep a Ways In section on research detail pages before treating active opportunities as the whole story.
...
- How should `/research` expose Ways In filters without turning Research into a job board?
- How should `/programs` balance fellowship deadlines, structured research programs, and planning support?
