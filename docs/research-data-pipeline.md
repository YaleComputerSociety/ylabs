...
Every other card surface batches the same read through the same function: the detail page's related and similar rails, and the saved-plan list in `researchPlanService.ts`.
A card surface that omits it is the defect, not a cheaper variant, so any projection feeding one must carry `rosterEnrichment`; the official-roster freshness filter inside the derivation fails closed without it and silently shortens the lead set.
