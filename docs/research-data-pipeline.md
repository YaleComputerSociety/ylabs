...
Measured on Development with both normalizations in place, over a 400-row sample of 4,744 live rows: 392 rows diverge in some field, but 227 of them diverge *only* in `unstorable` fields, leaving 165 rows (41 percent, about 1,957 at corpus scale) with any actionable drift at all.
Of those, `fill-empty` reaches 44 rows, `overwrite` 142 and `clear-stored` 19, so a blanket rematerialize would empty roughly 225 live rows to fill roughly 522 and more than half of the headline can never be closed by any run.
...
Re-measure with the census rather than quoting these figures back: the corpus moves, two of the classes are the thing a repair is meant to change, and a change to the materializer's read scope moves the `unstorable` occurrence counts without any row changing.
