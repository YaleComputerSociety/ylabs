...
Run `yarn --cwd server research-scope:backfill --mode=dry-run --output <artifact>` first, review the changed count and blocking reasons, then run `yarn --cwd server research-scope:backfill --mode=apply --confirm-student-visibility-apply --max-apply=<reviewedChangedCount> --output <artifact>` under the existing environment write guards.
