...
Both readers go through that one helper rather than restating the predicate, which retires an `|| reasons.length > 0` fallback in the audit script that was measured inert on Development: all 1,171 rows carrying no `studentVisibilityComputedAt` are archived, and every one of the 4,744 live rows carries it.
...
That stamp is deliberately carried in its own bulk-write op rather than folded into the visibility update, because the Meili resync is keyed on the ops that change what a student sees and folding it in would resync the whole evaluated scope on every gate run.
