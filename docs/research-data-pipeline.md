...
It delegates rather than re-applying one chosen stage, because naming a stage diverges from the card in both directions: the hygiene layer blanks bodies `publicResearchEntityDescriptionText` alone keeps, and the repair passes that run ahead of the blanking predicates rescue bodies a lone predicate call calls blank.
