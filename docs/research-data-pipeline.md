...
The `netid:`-shaped and email-shaped keys in that population needed a lead resolver that can reach an accountless researcher, not a looser mint, and #2763 gave them one.
...
The ordering is load-bearing, because the alias map states whose address the alias is while the alias only spells a name, and it fails closed on `ambiguous` for the #2675 reason.
