...
- Clean-to-clean refreshes are guarded too as of #2232: `isWeakerProseRefresh` also drops an incoming prose value that is useful but reads as a strictly worse statement of the home's research than the clean incumbent it would displace, because a flag-free-versus-flag-free contest was otherwise decided by the confidence gap alone. Ties pass, so a same-quality refresh keeps newest-wins. `collapseLatestWins` applies the same comparison so the preference also holds under `C4_LOSSLESS_INGEST`. [`skills/scrapers/SKILL.md`](../skills/scrapers/SKILL.md) owns the scoring rules and why the kind-aware person-centric term is excluded from them.
