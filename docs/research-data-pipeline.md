...
The public researcher profile's research-entity list (`publicProfileResearchEntity` in `profileService.ts`) is the one card surface still calling the sanitizer with no names, so it is running that smaller guard set today.
