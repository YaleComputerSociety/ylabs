...
The payload is a Yale email local part on 2,068 of 2,079 distinct values, and `materializeUser` projects that same value onto `identifiers.netid`, so the malformed value is the live join key between a researcher and their own observations rather than a dangling label: 490 researchers are joined to 13,407 observations by exactly that local part.
...
The honest fix for the false namespace therefore belongs at the readers, not the key: a consumer refuses to read a non-netid-shaped payload as a netid (`looksLikeYaleNetid` in [`server/src/utils/yaleNetid.ts`](../server/src/utils/yaleNetid.ts)), which is what #2864 did to release 159 leads without touching a single key.
