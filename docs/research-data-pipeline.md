...
The `user`-observation path in `entityMaterializer` mints a person only where the corpus already names that person as the lead of a live research entity: a bare directory identity that reaches no existing researcher by netid, name, or email, and that no live `inferredPiUserKey` observation names, is still refused with `skipped('directory-identity-without-research-signal')` (issue #2129).
...
Three conditions keep the mint from adding a record nobody can use: the name resolution must be `absent` rather than `ambiguous`, so a name the corpus already holds more than once is never duplicated; the attribution key must be the `dept:<ns>:<slug>` shape `materializeInferredPiMembership` derives a name from, because a `netid:`-shaped or email-shaped key resolves only through `identifiers.netid` or an `Account` and the mint may stamp neither; and the entity making the attribution must not be archived, because a folded dept-roster shell is archived without superseding its observations.
...
The `netid:`-shaped and email-shaped keys in that population are still held: unblocking them needs a lead resolver that can reach an accountless researcher, not a looser mint.
...
It composes the full guard union in a fixed, idempotent order: first `withoutAnotherOrganizationsProse`, then the text-transform layer (subjectless-lead repair, first-person re-voicing, mismatched-name-prefix correction, the non-person-org biography guard, and the `publicResearchEntityDescriptionText` fail-closed gate), then the faculty and research-home self-reference relabel passes, then the `descriptionHygiene` layer (chrome and dump stripping, contact-block/publications/center-blurb/HTML fail-close, and the per-field length clamps).
`withoutAnotherOrganizationsProse` withholds a person-scoped row's long body, and a card that restates that same prose, when the body makes a third-party organization its subject, so a department's or a core facility's prose is never served as one faculty member's research (#2480).
It has to run before the text-transform layer rather than alongside the others, because that layer relabels a person-scoped body's own research home ("The Smith Laboratory studies" -> "The Smith research program studies") and would hand the rule an organizational head noun it manufactured; the judgement belongs on the harvested prose.
Two residuals of that withhold are accepted rather than closed, and both follow from the rule living in the DTO while the detail route composes its copy upstream.
The detail route resolves an absent card out of the body at `resolveServedShortDescription`, which runs before the DTO, so a summary lifted from a body this rule then refuses can still reach the row whenever the lift paraphrased the organization out of the lead and left nothing for the card's containment or provenance arms to recognize.
The same ordering computes the `blank_served_public_description` invariant on the stored body, so a person-scoped row with no research-area chips can serve no prose at all once the DTO withholds both fields; the withhold is deliberately kept out of that gate, because failing it would 404 a row that still has a lead, official links and research areas rather than correct what the row says.
[`skills/scrapers/SKILL.md`](../skills/scrapers/SKILL.md) owns the subject-position rule itself and its write-time choke points.
...
Both skip a candidate the entity's stored `sourceLinkHealth` records as dead and fall through in confidence order, and the lead projection additionally refuses a candidate whose successor the entity already cites; `skills/scrapers/SKILL.md` owns those refusal rules.
...
Re-running `programs:backfill-classification` over the whole live catalog is not a safe way to deliver a classifier change. A recomputed classification only asserts the optional audience fields it has evidence for, and the script `$unset`s the rest, so a corpus-wide apply clears stored `undergraduateOnly` / `yaleCollegeOnly` on rows the classifier no longer speaks to and drops them out of the gate's `audienceKnown` branch; measured on Development that cost 78 rows their public tier. Use `--only-archive-review` to narrow the scan to stored `Archive / review` rows when the point of the run is to release records the classifier no longer archives, read `audienceFieldsCleared` in the dry-run report before any apply, and re-gate the affected records with `student-visibility:gate --collection=programs --record-id=...` rather than corpus-wide.
