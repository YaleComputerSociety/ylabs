...
The release queue is written by `yarn --cwd server student-visibility:gate`. Scraper `--auto-materialize`, manual materialize, and production cron paths run the gate after clean write materialization. Scheduled or manual global reconciliation should first run `yarn --cwd server student-visibility:gate --collection=research --mode=dry-run --output /tmp/<review>.json` and inspect `changedTierSummary` for public-to-held collapses, entity-type clusters, and blocker reasons before any `--mode=apply` run under the existing environment write guards.

Canonical naming repairs are narrow source-backed operations, not broad inference. Use `yarn --cwd server research-entity:canonical-name-repair` as a dry run before applying a known repair such as `dept-cs-lin-zhong` to `Efficient Computing Lab`. The script preserves the existing slug, records the old PI-generated label as a deferred alias note, and only applies when official lab and faculty sources support the identity.

Research activity enriches the lab/detail experience but is not itself undergraduate access evidence. Papers and scholarly links should appear when they are tied through canonical `research_scholarly_links` plus `research_scholarly_attributions`, legacy `paper_authors` identity proof, or explicit entity-paper links, helping students understand what the PI or lab studies before clicking through to a professor profile. Do not let name-only publication matches create PI/member links, access signals, contact routes, or public visibility.

`yarn --cwd server papers:quality-audit --sample-limit=0` is the read-only research-activity sanity gate. A result with zero legacy active papers is a coverage warning only when canonical scholarly links and attributions are also absent. On 2026-05-26 the current Beta target had `papers=0` and `paper_authors=0`, but canonical `research_scholarly_links` and `research_scholarly_attributions` were populated, so the release gate should verify canonical activity coverage and spot-check runtime pages instead of restoring legacy paper materialization solely for its own sake.
