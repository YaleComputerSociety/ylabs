...
Each extractor computes a SHA-256 hash over the exact fetched page bytes plus the extraction contract that would consume them (the extractor's prompt version and model id, and for the description extractor also the card model and card-synthesis prompt version), compares it against the last stored `sourceContentHash` bookkeeping observation for that `(source, entity)`, and skips the paid LLM call entirely when both the bytes and the contract are unchanged.
Because the contract is folded into the key, bumping a prompt version or changing a model re-extracts exactly the affected entities on the next run instead of silently serving output produced by the older prompt or model, while unchanged pages still skip.
