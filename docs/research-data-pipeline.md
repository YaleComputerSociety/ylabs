...
`servedFullDescription` decides it, applying `publicResearchEntityDescriptionText` and the #2480 organizational withhold, so a row storing an appointment dump, a role-only fragment, a contact route or another organization's prose counts as serving nothing.
