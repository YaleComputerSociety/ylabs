...
Which paths count as a person page is settled by `utils/yalePersonPagePrefix.ts` rather than by a second list in the dedupe core, so a host that nests a category between its person-page prefix and the person is seen (`environment.yale.edu/directory/faculty/<person>`, `som.yale.edu/faculty-research/faculty-directory/<person>`), and a host absent from that map stays invisible on purpose (#2750).
A root-mapped host is excluded, because an empty prefix asserts nothing about its leaf and every top-level page would read as somebody's profile.
Do not relax this to "any intermediate path segment", which #2750 also suggested: measured on Development, that variant claims 743 of 7,919 cited URLs as a person across 183 path shapes, led by 268 `medicine.yale.edu/lab/<leaf>` pages, 60 `research.yale.edu/cores/<leaf>` facilities and 54 `nsf.gov/awardsearch/<leaf>` grant pages.
Every false identity costs a correct merge, because an identity here only ever refuses one, and the lane-level dry-run cannot detect the damage: with both URL lanes near exhausted, even the fully permissive variant leaves both reports byte-identical.
