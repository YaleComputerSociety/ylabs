...
This table records public repository state only.
Private operational evidence is shown only as pending or accepted and must never be summarized here with database-derived counts, identifiers, artifact references, or protected evidence.
Repository-only schemas, tests, dry-run tooling, and documentation do not complete a phase and do not authorize its runtime cutover.

Phase 0 exit is a hard gate.
No Phase 1 through Phase 6 runtime reader redirect, Meilisearch cutover, target-collection write, compatibility-field removal, or storage deletion may proceed before [Phase 0](https://github.com/YaleComputerSociety/ylabs/issues/204) has privately reviewed and accepted its exit evidence.
After Phase 0 exits, the operational sequence is Phase 1, Phase 2, the remaining Phase 3 and Phase 4 work, Phase 5, and finally Phase 6.

| Phase                                                              | Repository foundation                                                                                                                                                                                                         | Private operational evidence                                                                                             | Runtime cutover                                                                                                                                                                             |
| ------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Phase 0](https://github.com/YaleComputerSociety/ylabs/issues/204) | **Substantial**: read-only inventory, identity-collision, recovery validation, search-baseline, query-cost, ownership, bounded admin access-review projection, and runbook foundations                                        | **Pending**: Development evidence review, Beta and ProductionCopy capture and review, collision-class decisions, and rollback acceptance | **Not applicable**: this phase measures and bounds the current system without redirecting canonical readers or writing target collections                                                    |
| [Phase 1](https://github.com/YaleComputerSociety/ylabs/issues/205) | **Substantial**: versioned canonical contracts, migration-safe validator operations, bounded loaders, and projections                                                                                                         | **Pending**: Development and Beta coexistence review plus a second validator dry run containing only no-op actions       | **Not started**: canonical readers remain isolated from live routes, materializers, and Meilisearch                                                                                         |
| [Phase 2](https://github.com/YaleComputerSociety/ylabs/issues/206) | **Foundation only**: canonical contracts, a bounded read-only identity, account, role, and quarantine planner, and fail-closed faculty-import identity guards exist                                                            | **Blocked**: private planner evidence, identity migration, and parity review wait for Phase 0 and Phase 1 exits          | **Not started**: public identity, leadership, and professor-name search still use compatibility models                                                                                      |
| [Phase 3](https://github.com/YaleComputerSociety/ylabs/issues/207) | **Partial**: bibliographic scraper implementations, active source metadata, schedules, release-gate invocations, hidden publication producers, repair writers, generic paper Observation materialization, public research-detail paper sections, the last profile, research-detail, and analytics paper payload readers, the publications route with its admin read and edit, the compatibility paper-count DTO fields, and the papers, paper_authors, and paper_entity_links promotion and sync allowlist entries are retired while curated OFFICIAL_PROFILE scholarly links, a guarded dry-run drop script kept as unexecuted tooling, and historical provenance and storage remain | **Pending**: outbound-link, search-parity, archive, and rollback acceptance                                              | **Paused at reversible preparation**: public UI no longer renders paper-derived sections, paper materializers fail closed by default, and the remaining server payload readers and promotion paths are removed, while audits, Meilisearch configuration, and storage remain |
| [Phase 4](https://github.com/YaleComputerSociety/ylabs/issues/208) | **Foundation only**: first-class access records and a bounded review-only legacy-record classification planner exist; orphan repair plus clean entity, taxonomy, and planning migrations do not                               | **Blocked**: orphaned-Observation repair implementation, reviewed Development evidence, and canonical entity, access, opportunity, and private-plan parity review wait for earlier exits | **Not started**: live entity, access, listing, fellowship, and saved-plan compatibility paths remain                                                                                        |
...
Use the [`Phase 2 identity-plan runbook`](./research-model-refactor-phase2-identity-plan.md) for the bounded read-only planner and private quarantine report.
The planner performs no target-collection writes and does not authorize the runtime cutover.
...
- active person source scrapers under `server/src/scrapers/sources/`;
