...
The `faculty_members` collection is left in place for a gated drop; the `User.facultyMemberId`, `Grant.piFacultyMemberId`/`coPiFacultyMemberIds`, `Paper.facultyMemberIds`, and `PaperAuthor.facultyMemberId` fields keep their stored `ObjectId`s but no longer declare a `ref` to the removed model.
