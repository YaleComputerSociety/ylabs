...
| ------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Phase 0](https://github.com/YaleComputerSociety/ylabs/issues/204) | **Substantial**: read-only inventory, identity-collision, recovery validation, search-baseline, query-cost, ownership, and runbook foundations                                                                                | **Pending**: reviewed Development, Beta, and ProductionCopy evidence, collision-class decisions, and rollback acceptance | **Not applicable**: this phase measures the current system without redirecting readers or writing target collections                                                                        |
...
| [Phase 2](https://github.com/YaleComputerSociety/ylabs/issues/206) | **Foundation only**: canonical contracts plus a bounded read-only identity, account, role, and quarantine planner exist                                                                                                       | **Blocked**: private planner evidence, identity migration, and parity review wait for Phase 0 and Phase 1 exits          | **Not started**: public identity, leadership, and professor-name search still use compatibility models                                                                                      |
| [Phase 3](https://github.com/YaleComputerSociety/ylabs/issues/207) | **Partial**: bibliographic scraper implementations, active source metadata, schedules, release-gate invocations, the hidden department-roster publication producer, its standalone repair writer, and generic paper Observation materialization are retired while historical provenance and storage remain | **Pending**: outbound-link, search-parity, archive, and rollback acceptance                                              | **Paused at reversible preparation**: official-profile and generic paper Observation materializers fail closed by default, but public scholarly readers, audits, promotion paths, Meilisearch configuration, and storage remain |
