...
| P2       | Add a minimal E2E smoke in CI or scheduled Beta checks.                          | Landed: the `E2E Smoke` job (`.github/workflows/e2e-smoke.yml`) runs on every PR, standing up an ephemeral Mongo + Meilisearch + API/static-client stack seeded with synthetic entities (`server/src/scripts/seedE2eSmokeData.ts`) and driving `scripts/e2e-student-journey-smoke.mjs` through browse -> search -> detail -> save plus an honest zero-result empty state, with no Production/Beta writes.                                                                                                     |
