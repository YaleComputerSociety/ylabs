...
6. Keep the student UX research-home-first: `/research/:slug` is the primary page for the lab/entity, PI, ways in, and relevant papers; professor profiles are supporting deep-dives, not the main discovery funnel.
...
- 2026-05-25: Added `yarn --cwd server papers:quality-audit --sample-limit=0` as a read-only paper activity sanity gate. Zero active papers now fails the paper quality pass state with a coverage warning so operators verify the materialization target DB and active-paper filter before relying on research activity.
...
- [x] Repair the Peters Lab/JDP52 data-quality failure. Listing-backed ResearchEntity creation now verifies owner `_id` and `netid` before attaching PI membership, listing profile repair no longer promotes person-page publication blurbs into entity descriptions, and the bad `peters-lab-jdp52` copied blurb was removed from Development so the record is held until it has a source-backed research description.
- [x] Add the V1 evidence coverage pipeline for research listing quality. `yarn scrape run --source <name> --dry-run --db-review` now captures dry-run observations in memory, compares them against current research entity claim coverage, reports resolved/remaining blockers in `evidenceCoverageImpact`, and surfaces thin/partial research entity repair pressure plus suggested source types on the Operator Board.
- [x] Solidify the research-home-first paper/activity UX direction. Lab/entity detail stays the primary student surface, with verified PI/member research activity as context for understanding the work and preparing outreach; profile research tabs remain secondary deep-dives.
- [ ] Expand verified paper/activity coverage for strong lab pages by prioritizing identity-backed `paper_authors` and explicit entity-paper links, then check the detail page for thin or misleading descriptions before mass-applying scraper writes.
