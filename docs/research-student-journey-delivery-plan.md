...
- **Subsequent state:** the saved-research routes referenced across CP-01, CP-02, and CP-05 have since cut over from the embedded `User` planning fields and `userService` to the canonical `ResearchPlan` collection through `researchPlanService`, with the one-time legacy migration retired in favor of a pending dry-run-first backfill; the source references in those entries record their state as shipped, and [`research-model.md`](./research-model.md#saved-research-entities) owns the current runtime contract.
...
- **Validation evidence:** PRs `#154` and `#169` completed major Programs and onboarding slices, and `#171` simplified Research.
  ...
  Rendered-surface a11y suites cover the previously unaudited canonical surfaces in loaded, empty, and error states: the research-home detail page (`/research/:slug`, including related and "More like this" regions), the fellowships list and its detail modal, and the account dashboard (saved research plans, the comparison dialog, and the watched-program export surface).
  The standalone faculty profile page (`/profile/:netid`) was retired; a person's public identity now surfaces through their research-entity membership and the external official-profile PI link, not a Yale-hosted profile page.
