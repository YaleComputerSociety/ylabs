...
- Description: `missing_description`, `missing_card_description`, `thin_description`, `blank_public_description`. A card that renders no real prose, or prose about something else. Maps to `descriptionCoherent` (and `entityContentMatchesCard` for off-entity content, e.g. a "<Person> Lab" name typed as an org whose body describes a center).
  - The lab-style "Studies X" card (`missing_card_description`) is required only for PI-led labs and faculty research areas. Program-like homes (described by what they offer and how to apply) and organizational homes (centers, institutes, initiatives, core facilities, library collections, archive/museum and digital-humanities projects - described by what the home is and does) are card-optional: a useful full description alone satisfies the description floor for them. The full-description requirement and the non-blank served-copy guard still apply. This card exemption is driven by `researchEntityCardIsOptional` in `server/src/utils/researchEntityProgramLike.ts`, so the serve-time card invariant and the gate stay in sync.
...
Later, the lab-style card exemption already granted to program-like homes was extended to organizational homes (centers, institutes, initiatives, core facilities, library collections, archive/museum and digital-humanities projects).
These homes are lead-exempt (issue #1367) and are described by what the home is and does rather than a lab-style "Studies X" research focus, yet they were still held on `missing_card_description` because the card exemption never covered their types.
The shared predicate `researchEntityCardIsOptional` now drives both the serve-time card invariant and the gate, so a well-described organizational home is no longer stranded for lacking a lab-style card.
