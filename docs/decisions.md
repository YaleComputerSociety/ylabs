...
The stranding guard is the load-bearing part and it was measured rather than reasoned about (#2630): 333 of those rows cite a roster and nothing else, so retracting it would trade a duplicate-URL block for a missing-evidence block, and the stage refuses on the roster arm alone while deliberately stranding a row whose only citation is a CMS loader endpoint that was never a readable page.
