...
Two independent instruments agree on the cohort, which is what makes the number usable: the script's own planner reports 942 rows and the new stage reports 941 on the stored list alone, the one row being the script's separate `websiteUrl` arm.
...
The magnitude of what it protects is #2630's: 318 rows stranded corpus-wide by the roster arm alone.
...
One over-reach worth naming because its population was measured rather than argued: `isSharedPeopleRosterUrl` matches a collective leaf token, so a lab's own `/lab-members` page on its own host is roster-shaped, and retracting it would remove a real citation.
0 of the 1,274 rows in the cohort cite a graft that sits inside their own stored `websiteUrl`, so the shape exists and the defect does not, which is why the stage carries no host-ownership exemption.
The obvious exemption would also not work: 256 rows do cite a graft on the same host as their `websiteUrl`, and that host is a shared CMS such as `medicine.yale.edu` rather than a host the row owns, so host ownership derived from `websiteUrl` would exempt most of the cohort it exists to correct.
