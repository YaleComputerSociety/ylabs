...
The guard, the `--max-apply` cap, and the plan the op applies are all resolved once inside `retireProgramResearchEntities`, and the search deletion reports whether Meilisearch actually accepted it rather than counting attempts.
...
The guard deliberately asks for the resolved winner rather than for any retained observation: `entityType` keeps a value-bearing fingerprint, so a stale `PROGRAM` row is never superseded by a later `INITIATIVE` assertion and matching it would permanently freeze every live entity whose type has since healed.
