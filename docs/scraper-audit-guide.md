...
Most scrapers write raw `Observation` rows first. `entityMaterializer.ts` then upserts physical entities such as `research_entities` and `users`. `accessMaterializer.ts` derives access-model records such as `entry_pathways`, typed `Signal` rows in `signals`, and `contact_routes` from evidence-bearing observations.
...
- Starts from existing access `Signal`, `EntryPathway`, public `ContactRoute`, and `PostedOpportunity` evidence; it does not scrape new pages.
...
- `entry_pathways`, `signals`, and guarded `contact_routes` through access materialization.
...
- They do not create `entry_pathways` or `signals` from index-only evidence.
