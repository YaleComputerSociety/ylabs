...
| `yale-directory`                   | Authoritative Yale appointment/profile metadata                                                                                       | `user`                                                                 | `researchers` (enrichment only)                                                                     | Membership/profile context only. Not access evidence.                                                                                                                                               | Depends on Yalies/API configuration. Good for improving person matching. Bare directory identity still mints no `Researcher` or `Account`, but a PI attribution counts as the research signal (#2773): an identity a live research entity's `inferredPiUserKey` observation names by exact `dept:<ns>:<slug>` key, and that no existing researcher answers to by name, mints an accountless `Researcher`, and an unattributed one only enriches a researcher a research signal already attached, filling profile fields and stamping only the linked account's own netid on `Researcher.identifiers.netid` (skips `directory-identity-without-research-signal`); `docs/research-model.md` owns the identity-join and netid-stamping rules. Accounts are created only at login.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
