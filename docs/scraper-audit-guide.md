...
- `signals`: typed `Signal` rows for evidence-backed access clues (each former access `signalType` is its own `Signal.type`; the five undergraduate-logistics claim types are retired, #3088).
...
The user-identity enrichment path also counts one when a source-resolved ORCID or netid is already claimed by another `Researcher`: the existing holder keeps that identifier, the enriched researcher keeps the identity it already had, and the collision is logged rather than failing the run.
Such a conflict has no matching sample in the review, so the run log line naming the colliding ORCID or netid is the signal that upstream directory data carries a duplicate to fix at the source.
...
This map covers what is scraped.
What was deliberately not scraped, and why, is recorded in [`docs/scraper-coverage-decisions.md`](./scraper-coverage-decisions.md), because a rejected source leaves no config behind and so reads here exactly like a source nobody has looked at yet.
...
| `lab-microsite-undergrad-llm`      | Extract evidence from lab/faculty websites: join pages, role language, constraints, and contact instructions | `researchEntity`                                                       | `research_entities` and access records when exact evidence supports them | Can create access `signals` through validated materializers. Bare join-page URLs without undergrad access evidence should not create access signals. | Start with small `--limit`. Requires `OPENAI_API_KEY`. Review every quote against its exact `quoteSourceUrl` and inspect freshness.                                                                                                                                                                                                                                                                                                   |
...
| `yale-directory`                   | Authoritative Yale appointment/profile metadata                                                                                       | `user`                                                                 | `researchers` (enrichment only)                                                                     | Membership/profile context only. Not access evidence.                                                                                                                                               | Depends on Yalies/API configuration. Good for improving person matching. Bare directory identity still mints no `Researcher` or `Account`, but a PI attribution counts as the research signal (#2773): an identity a live research entity's `inferredPiUserKey` observation names by exact key, and that no existing researcher answers to by name, mints an accountless `Researcher` (the key shapes that may mint are owned by `docs/research-data-pipeline.md`), and an unattributed one only enriches a researcher a research signal already attached, filling profile fields and stamping only the linked account's own netid on `Researcher.identifiers.netid` (skips `directory-identity-without-research-signal`); `docs/research-model.md` owns the identity-join and netid-stamping rules. Accounts are created only at login.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `nih-reporter`                     | Pull Yale NIH grants and PI/co-PI context                                                                                             | `researchEntity` grant-shaped observations on an existing row           | `researchers`, `research_entities`, `grants`                                                         | Funding/topic enrichment only. Not undergraduate access evidence.                                                                                                                                   | Audit PI matching, the not-attached reasons in run notes, grant counts, and duplicate external IDs.                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `nsf-award-search`                 | Pull Yale NSF awards and PI/co-PI context                                                                                             | `researchEntity` grant-shaped observations on an existing row           | `researchers`, `research_entities`, `grants`                                                         | Funding/topic enrichment only. Not undergraduate access evidence.                                                                                                                                   | Especially useful where department pages are JS-heavy.                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
- `observations`: evidence-shaped access fields, description text, and the quote fields that back them.
