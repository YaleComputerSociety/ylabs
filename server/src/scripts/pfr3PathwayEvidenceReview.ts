...
function readJson(file: string): unknown {
  ...
  if (!stat.isFile() || stat.size > 256 * 1024)
    throw new Error('input JSON must be a file no larger than 256 KiB');
...
async function main(): Promise<void> {
  ...
  const privateOutput = resolveSafeJsonReportOutputPath(
    value(args, 'private-output'),
    '--private-output',
  );
  await initializeConnections();
  const candidates = await EntryPathway.find({ archived: { $ne: true } })
    .select(
      '_id researchEntityId status evidenceStrength confidence sourceUrls sourceEvidenceIds lastObservedAt',
    )
    .lean();
  const selected = resolveReviewCandidates(
    candidates.map((item: any) => ({ ...item, id: item._id })),
    handlesInput,
    salt,
    maxBatch,
  );
  const artifact = selected.map((item) => ({
    ...
    sourceEvidenceIds: Array.isArray(item.sourceEvidenceIds)
      ? item.sourceEvidenceIds.map(String)
      : [],
  ...
  fs.writeFileSync(
    privateOutput,
    JSON.stringify({ classification: 'PRIVATE', target, artifactHash, records: artifact }, null, 2),
    { mode: 0o600, flag: 'w' },
  );
  ...
  if (value(args, 'decisions')) {
    ...
    decisionArtifactHash =
      typeof envelope?.artifactHash === 'string' ? envelope.artifactHash : undefined;
    decisions = validateReviewDecisions(
      envelope?.decisions ?? raw,
      new Set(artifact.map((item) => item.handle)),
    );
    if (decisionArtifactHash && decisionArtifactHash !== artifactHash)
      throw new Error('decision artifact hash does not match the selected salted artifact');
    if (execute && !decisionArtifactHash)
      throw new Error('execute requires the decision artifact hash');
  }
  let applied = {
    applied: 0,
    idempotent: 0,
    manualOnly: decisions.filter((item) => item.disposition === 'manual_only').length,
    rejected: 0,
  };
  if (execute && decisions.length) {
    ...
    applied = await applyValidatedPathwayDecisions({
      ...
      candidates: new Map(
        artifact.map((item) => [
          item.handle,
          {
            ...
            researchEntityId: item.researchEntityId,
      ...
      restoreToken: value(args, 'restore-token')!,
      deps: {
        findEvidence: async (ids) =>
          (await Observation.find({ _id: { $in: ids } }).lean()).map((item: any) => ({
            id: serializedDocumentId(item._id) || '',
            entityType: item.entityType,
            entityId: serializedDocumentId(item.entityId),
            entityKey: item.entityKey,
            field: item.field,
            value: item.value,
            sourceId: serializedDocumentId(item.sourceId) || '',
            sourceName: item.sourceName,
            sourceUrl: item.sourceUrl,
            confidence: item.confidence,
          })),
        alreadyApplied: async (key) =>
          Boolean(await audit.findOne({ key }, { projection: { _id: 1 } })),
        appendEvidence: async (evidence, sourceUrl) => {
          const source = await getSourceByName(evidence.sourceName);
          if (!source || source._id !== evidence.sourceId)
            throw new Error('source evidence provenance no longer matches');
          const run = await ScrapeRun.create({
            sourceId: source._id,
            sourceName: source.name,
            triggeredBy: 'admin',
            options: { workflow: 'pfr3-pathway-evidence-review' },
          });
          const result = await appendObservations(
            ...
            ],
            {
              ...
              dryRun: false,
              scrapeRunId: serializedDocumentId(run._id) || '',
            },
          );
          await ScrapeRun.updateOne(
            { _id: run._id },
            {
              $set: {
                status: 'success',
                finishedAt: new Date(),
                observationCount: result.inserted,
                entitiesObserved: 1,
              },
            },
          );
        },
        materialize: async (researchEntityId) => {
          await materializeAccessForResearchGroup({ researchEntityId });
        },
        writeAudit: async (row) => {
          await audit.insertOne({ ...row, createdAt: new Date() });
        },
      },
    });
  }
