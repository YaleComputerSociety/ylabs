...
const ROLLBACK_REASON =
  'grafted directory url: a CMS internal endpoint is never a readable page, and a departmental programme page is evidence about a department rather than about a person (#2605, #2609, #2611)';

export interface RetireGraftedDirectoryUrlsArgs {
  apply: boolean;
  confirm: boolean;
  maxApply: number;
  output?: string;
...
export function parseArgs(argv: string[]): RetireGraftedDirectoryUrlsArgs {
  const args: RetireGraftedDirectoryUrlsArgs = {
    apply: false,
    confirm: false,
    maxApply: 120,
  };
  for (let index = 0; index < argv.length; index++) {
    ...
    else if (arg === '--confirm-retire-grafted-directory-urls') args.confirm = true;
...
export async function loadPlannedEntities(): Promise<PlannedEntity[]> {
  ...
  for (const entity of entities as any[]) {
    const plan = planGraftedUrlRepair(entity);
    if (!plan) continue;
...
export async function loadPlannedObservationIds(
  slugs: string[],
  entityBySlug: Map<string, { entityType?: unknown }> = new Map(),
): Promise<string[]> {
  ...
  return (observations as any[])
    .filter((obs) =>
      isGraftValuedObservation(obs.field, obs.value, entityBySlug.get(obs.entityKey) || {}),
    )
...
async function applyRepair(
  ...
  // Removing a citation changes what the gate had to work with, so any row that lost
  // one is re-decided rather than left on a decision made about evidence it no longer
  // has. Three rows in this cohort end with no citation at all, two of them served.
  const regateIds = planned
    .filter((entry) => entry.plan.removedSourceUrls.length > 0 || entry.plan.clearWebsiteUrl)
...
async function main() {
  ...
  if (args.apply) {
    if (!args.confirm) {
      throw new Error('--confirm-retire-grafted-directory-urls is required when --apply is set.');
    }
