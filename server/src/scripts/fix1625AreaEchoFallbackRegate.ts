...
const confirmed = process.argv.includes('--confirm-fix-1625');
...
async function main(): Promise<void> {
  ...
  if (apply && !confirmed) {
    console.error('--confirm-fix-1625 is required when --apply is set.');
    process.exitCode = 1;
    return;
  }
  ...
  console.error(
    `Found ${recordIds.length}/${candidates.length} student_ready entities matching the fluent area-echo fullDescription shape (#1625)`,
  );
  ...
  console.error(
    'Re-gate against the fluent multi-sentence area-echo fallback template now recognized by fullDescriptionQuality (#1625):',
  );
...
main()
  .catch((error) => {
    console.error('fix1625AreaEchoFallbackRegate failed:', error);
    process.exitCode = 1;
  })
  .finally(async () => {
