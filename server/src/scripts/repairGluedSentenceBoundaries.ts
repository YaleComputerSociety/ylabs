...
import {
  ...
  type GluedSentenceBoundaryRepairRow,
...
export interface GluedSentenceBoundaryCliOptions {
  dryRun: boolean;
  confirm: boolean;
  output?: string;
}

export function parseGluedSentenceBoundaryArgs(
  argv: readonly string[],
): GluedSentenceBoundaryCliOptions {
  const options: GluedSentenceBoundaryCliOptions = { dryRun: true, confirm: false };
...
export interface GluedSentenceBoundaryResult {
  mode: 'dry-run' | 'apply';
  scanned: Record<string, number>;
  summary: ReturnType<typeof summarizeGluedSentenceBoundaryRepair>;
  documentsUpdated: number;
  entitiesResynced: number;
  entitiesAwaitingResync: number;
  rows: Array<Omit<GluedSentenceBoundaryRepairRow, 'set'>>;
}

export async function runGluedSentenceBoundaryRepair(options: {
  dryRun: boolean;
}): Promise<GluedSentenceBoundaryResult> {
  ...
  // strict Mongoose `$set` drops it silently (#2874).
  const db = mongoose.connection.db;
  if (!db) throw new Error(`${SCRIPT_NAME} requires a connected database`);
  const scanned: Record<string, number> = {};
  const rowsByCollection = new Map<string, GluedSentenceBoundaryRepairRow[]>();
  for (const collection of REPAIRED_COLLECTIONS) {
    const documents = await db.collection(collection).find({}).toArray();
    scanned[collection] = documents.length;
    rowsByCollection.set(
      collection,
      planGluedSentenceBoundaryRepair(collection, documents as Array<Record<string, unknown>>),
    );
  }
  const rows = REPAIRED_COLLECTIONS.flatMap((collection) => rowsByCollection.get(collection) ?? []);

  const result: GluedSentenceBoundaryResult = {
    mode: options.dryRun ? 'dry-run' : 'apply',
    scanned,
    summary: summarizeGluedSentenceBoundaryRepair(rows),
    documentsUpdated: 0,
    entitiesResynced: 0,
  ...
  // index keeps serving the glued form the corpus no longer holds. `syncEntities`
...
async function main(): Promise<void> {
  const options = parseGluedSentenceBoundaryArgs(process.argv.slice(2));
  ...
  try {
    const result = await runGluedSentenceBoundaryRepair({ dryRun: options.dryRun });
    if (options.output) {
      ...
      console.log(`Saved glued-sentence-boundary repair report to ${safeOutput}`);
    }
    console.log(JSON.stringify({ ...result, rows: undefined }, null, 2));
    if (result.entitiesAwaitingResync > 0) {
      throw new Error(
        `${SCRIPT_NAME} repaired the corpus but ${result.entitiesAwaitingResync} search document(s) were not resynced; the index still serves the glued text.`,
      );
    }
  } finally {
