...
const confirmed = process.argv.includes('--confirm-fix-1511');
...
async function main(): Promise<void> {
  ...
  if (apply && !confirmed) {
    console.error('--confirm-fix-1511 is required when --apply is set.');
    process.exitCode = 1;
    return;
  }
  ...
  console.error(`Re-gate against the corrected "is connected to" keyword-list-stub detector:`);
...
main()
  .catch((error) => {
    console.error('fix1511ConnectedToStubRegate failed:', error);
    process.exitCode = 1;
  })
  .finally(async () => {
