...
const confirmed = process.argv.includes('--confirm-fix-1798');

// Scoped to exactly the entities the #1798 department-name alias unblocks
// (English Language and Literature -> Literature, WGSS -> Gender Studies).
// Deliberately excludes solomon-pag55 and glover-klg68 (also missing_facet_signal,
// also Black Studies department), which have no clean existing-taxonomy
// naming-variant counterpart and are held pending a separate product-model call.
const TARGET_SLUGS = [
  'patterson-annabel',
  'campbell-jc5',
  'levine-ngl3',
  'feimster-cf84',
  'dept-english-sunny-xiang',
];
...
async function main(): Promise<void> {
  ...
  if (apply && !confirmed) {
    console.error('--confirm-fix-1798 is required when --apply is set.');
    process.exitCode = 1;
    return;
  }
  ...
  console.error(
    `Research-area backfill via #1798 humanities department-name aliases: ${changedAreaRows.length}/${areaRows.length} would gain areas`,
  );
  changedAreaRows.forEach(logAreaChange);

  const unchanged = areaRows.filter((row) => !row.changed);
  if (unchanged.length > 0) {
    console.error(
      `No area derived (held on missing_facet_signal pending #1798 follow-up): ${unchanged
  ...
  const changedRecordIds = changedAreaRows.map((row) => row.id);
  ...
  console.error(`Re-gate against the newly-derived research areas (#1798):`);
  ...
  if (apply) {
    ...
    const fresh = await ResearchEntity.find({
      _id: { $in: changedRecordIds.map((id) => new mongoose.Types.ObjectId(id)) },
    }).lean();
...
main()
  .catch((error) => {
    console.error('fix1798HumanitiesDeptAliasFacetGate failed:', error);
    process.exitCode = 1;
  })
  .finally(async () => {
