...
const MAX_CANDIDATES_PER_SUBJECT = 8;
 ...
 */
async function search(queries: string[]): Promise<string[]> {
  ...
  if (parallel) {
    const response = await fetch('https://api.parallel.ai/v1/search', {
      ...
      body: JSON.stringify({
        objective: LAB_SITE_SEARCH_OBJECTIVE,
 ...
 */
async function searchWithRetry(queries: string[]): Promise<string[]> {
  let lastError: unknown;
  for (let attempt = 1; attempt <= SEARCH_ATTEMPTS; attempt += 1) {
    try {
      return await search(queries);
    } catch (error) {
      lastError = error;
      if (
...
async function run(
  ...
  for (const subject of selected) {
    let urls: string[];
    try {
      urls = await searchWithRetry(subject.queries);
