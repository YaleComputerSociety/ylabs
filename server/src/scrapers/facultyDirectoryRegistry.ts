...
export const FACULTY_DIRECTORY_REGISTRY: FacultyDirectoryEntry[] = [
  ...
  // ---- Tier 2: YSM basic-science departments (#1629) ------------------------------
  {
    ...
    approxFacultyCount: 124,
    notes:
      'Covered via dept-faculty-roster (#1629) on the shared YSM profile-grid-item Drupal theme (the same extractor path as the Stem Cell Center and YCGA), each card linking the faculty member\'s own medicine.yale.edu/cellbio/profile/<slug>/ page. Wired official-profile-only: the school-wide ysm-faculty-directory A-Z crawl already mints each PI\'s research home at the school level, so this lane cites the profile as an official source and stamps the Cell Biology department onto the PI without minting a duplicate lab entity. The roster root is only a crawl seed, never a cited Source/Observation (#516/#549). This is the full department people page (faculty, research scientists, trainees, staff); only research-active profiles materialize a research home. Expect heavy dedup against the A-Z discovery and FAS joint appointments.',
  },
  {
  ...
  },
  {
    ...
    coveredBy: ['dept-faculty-roster'],
    studentImpactTier: 2,
    approxFacultyCount: 304,
    notes:
      'Department people roster (all roles on one page). Distinct from the YCGA staff roster (medicine.yale.edu/genetics/research/ycga/people/), which is a strict subset covered separately as a center by centers-institutes-index; this row attributes the Genetics department itself.',
  },
  {
    ...
    notes:
      'Cellular & Molecular Physiology exposes a dedicated faculty-scoped roster at /faculty/ rather than a combined /people/ page, so the faculty subset is crawled directly.',
  },
  {
    ...
    coveredBy: ['dept-faculty-roster'],
    studentImpactTier: 2,
    approxFacultyCount: 14,
    notes:
      'Microbial Pathogenesis splits its people directory into role-scoped subpages. The primary-faculty roster (~14) and the research-faculty roster (~12, medicine.yale.edu/micropath/people/research-faculty/) are both wired as dept-faculty-roster rows since both are department-housed research faculty. The secondary-faculty subpage is evaluated-skipped: those are joint appointments whose primary department is elsewhere, so stamping them Microbial Pathogenesis would mis-attribute their home.',
  },
  {
    ...
    notes: 'Research-track faculty companion to the primary-faculty roster; see that entry.',
  },
  {
  ...
  },
  {
    ...
    coveredBy: ['dept-faculty-roster'],
    studentImpactTier: 2,
    approxFacultyCount: 174,
    notes:
      'Large combined clinical + research department roster; the department attribution and official-profile citation apply to every research-active profile, and non-research clinical/staff entries never materialize a research home.',
  },
  {
    ...
    notes:
      'Heavily cross-listed with FAS (MCDB, Psychology) and other YSM departments; expect meaningful dedup against home-department appointments.',
  },
  {
  ...
  },
  {
    ...
    paginated: true,
    notes:
      'Covered via six dept-faculty-roster discipline config rows (som-accounting, som-economics, som-finance, som-marketing, som-operations, som-organizational-behavior) that crawl each of SOM\'s six research-discipline sub-pages (#1377). Each renders server-side with the Drupal node-teaser--faculty theme, so all reuse nodeTeaserFacultyExtractor. Crawling per discipline (rather than the paginated directory root as originally landed under #1293) captures each ladder faculty member\'s real discipline as their department instead of the coarse school-level "Management" bucket, so the department facet is navigable below the school level. Each card links the faculty member\'s own som.yale.edu/faculty-research/faculty-directory/<slug> page, which is cited as the per-faculty source; the discipline pages are only crawl seeds. The discipline pages cover the ladder faculty (the research-active cohort); non-ladder directory entries (lecturers, practice faculty, senior fellows) are out of scope for discipline capture.',
  },
  {
