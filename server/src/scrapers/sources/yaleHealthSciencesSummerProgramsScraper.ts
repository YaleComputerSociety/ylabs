/**
 * Yale health-sciences undergraduate summer research program scraper.
 *
 * Yale School of Medicine, Yale School of Public Health, Yale School of Nursing,
 * and their institutes/centers run application-based, deadline-driven summer
 * research programs that admit undergraduates - often first-generation,
 * underrepresented, or visiting students - into biomedical, clinical, and
 * public-health research. These are the biomedical analogue of the NSF-REU lane
 * (yaleReuProgramsScraper): a concrete, application-based on-ramp for the
 * undergraduate who does not already have a faculty connection, discovered on a
 * distinct, non-overlapping set of Yale health-sciences host domains.
 *
 * Each program's own official Yale page is cited as the source. Yale-owned
 * health-sciences listing/index pages are used only as crawl seeds to discover
 * individual program pages and are never recorded as a source (self-referential /
 * index-page source guards #516/#549). Contact is fail-closed - no scraped emails
 * are emitted; the read-time layer derives contact from the official page.
 * Application portals (the external apply link) are recorded as a link, never
...
export interface HealthSciencesProgramSeed {
  url: string;
  hostingOffice: string;
}

/**
 * Curated, reviewable seed of Yale health-sciences undergraduate summer research
 * program pages. Each entry's `url` is the program's own official Yale page and is
 * cited as the source; `hostingOffice` is a display label, not a contact route.
 * Grow this list as the health-sciences listing pages surface more programs.
 */
export const CURATED_YALE_HEALTH_SCIENCES_SUMMER_PROGRAM_SEEDS: HealthSciencesProgramSeed[] = [
  {
    url: 'https://medicine.yale.edu/biomedsurf/',
    hostingOffice: 'Yale School of Medicine - Biomedical Sciences (BioMed SURF / Amgen Scholars)',
  },
  {
  ...
  },
  {
    ...
    hostingOffice: 'Yale Cooperative Center of Excellence in Hematology (Laboratory Medicine)',
  },
  {
    url: 'https://ysph.yale.edu/school-of-public-health/special-programs/big-data-summer-immersion-at-yale/',
    hostingOffice: 'Yale School of Public Health - Biostatistics',
  },
  {
    url: 'https://ysph.yale.edu/school-of-public-health/special-programs/summer-research-experience-environmental-health/',
    hostingOffice: 'Yale School of Public Health - Environmental Health Sciences',
  },
];

/**
 * Yale-owned health-sciences listing/index pages parsed only to discover
 * individual program URLs. These roots are never emitted as a source citation
 * (self-referential / index-page source guards #516/#549).
 */
export const YALE_HEALTH_SCIENCES_DIRECTORY_SEED_URLS: string[] = [
  'https://ysph.yale.edu/school-of-public-health/special-programs/',
];

/**
 * Programs already minted by the Yale College Fellowships Office source
 * (yaleCollegeFellowshipsOfficeScraper DEFAULT_PAGE_URLS). Excluded from both the
 * seed set and directory discovery so a program is never minted twice under two
 * different source keys (#1712 de-dup requirement).
 */
export const EXCLUDED_ALREADY_COVERED_URLS = [
...
interface YaleHealthSciencesSummerProgramsScraperDeps {
  programSeeds?: HealthSciencesProgramSeed[];
...
export function sourceKeyForTitle(title: string): string {
  return `${YALE_HEALTH_SCIENCES_SUMMER_PROGRAMS_SOURCE}:${slugify(title)}`;
}
...
const EXCLUDED_URL_SET = new Set(EXCLUDED_ALREADY_COVERED_URLS.map(normalizeUrl));

export function isExcludedAlreadyCoveredUrl(url: string): boolean {
  return EXCLUDED_URL_SET.has(normalizeUrl(url));
}

export function isHealthSciencesSummerProgramUrl(url: string): boolean {
  try {
    const pathname = new URL(url).pathname.toLowerCase();
    if (/scholarship|charitable|financial-aid|donors|giving|spotlight|alumni/.test(pathname)) {
      return false;
    }
    return /summer|surf|reu|research-experience|internship|scholars(?:$|[-/])|pipeline|step-?up|undergraduate-research/.test(
      pathname,
    );
  } catch {
    return false;
...
/**
 * Parse one Yale health-sciences program page into a candidate. Returns undefined
 * for a non-Yale page (the source citation must be Yale-owned), a page with no
 * usable title, or a page whose prose carries no undergraduate summer-research
 * signal.
 */
export function parseHealthSciencesProgramPage(
  html: string,
  pageUrl: string,
  hostingOffice: string,
  referenceDate: Date = new Date(),
): HealthSciencesProgramCandidate | undefined {
  ...
  const hasSummerResearch =
    /research experiences? for undergraduates|\bnsf reu\b|\breu\b|summer undergraduate research|summer research (?:program|fellowship|scholars?|internship)|\bsurf\b|amgen scholars|\bstep[- ]?up\b/i.test(
      identityText,
    ) ||
    (/\bsummer\b/i.test(identityText) &&
      /\bresearch|internship|scholars?\b/i.test(identityText) &&
      /\bundergraduate|any institution|visiting students?|first[- ]generation|underrepresented|college students?\b/i.test(
        identityText,
      ));
  ...
  const candidate: Omit<HealthSciencesProgramCandidate, 'sourceFingerprint'> = {
    sourceKey: sourceKeyForTitle(title),
    title,
    description,
    eligibility,
    competitionType: 'Summer Undergraduate Research Program (Health Sciences)',
    sourceUrl: pageUrl,
    applicationLink,
    links,
    deadline,
...
/**
 * Parse a Yale-owned health-sciences listing page for links to individual
 * program pages. The listing root is a crawl seed only and is never cited; only
 * Yale-owned, HTML-like, program-shaped, non-excluded links are returned.
 */
export function extractProgramUrlsFromDirectory(html: string, pageUrl: string): string[] {
  const $ = cheerio.load(html);
  $('script, style, noscript').remove();
  const directoryUrl = normalizeUrl(pageUrl);
  const urls = new Set<string>();
  for (const link of $('a').toArray()) {
    const rawUrl = absoluteUrl($(link).attr('href'), pageUrl);
    if (!rawUrl) continue;
    const url = normalizeUrl(rawUrl);
    if (url === directoryUrl) continue;
    if (isExcludedAlreadyCoveredUrl(url)) continue;
    if (!isYaleOwnedUrl(url) || !isHtmlLikeUrl(url) || !isHealthSciencesSummerProgramUrl(url)) {
      continue;
    }
    urls.add(url);
  }
...
async function fetchHtml(url: string, useCache: boolean): Promise<string> {
  ...
  if (useCache) {
    const cached = await getCached<string>(YALE_HEALTH_SCIENCES_SUMMER_PROGRAMS_SOURCE, cacheKey);
...
export class YaleHealthSciencesSummerProgramsScraper implements IScraper {
  readonly name = YALE_HEALTH_SCIENCES_SUMMER_PROGRAMS_SOURCE;
  readonly displayName = 'Yale Health-Sciences Undergraduate Summer Research Programs';

  private readonly programSeeds: HealthSciencesProgramSeed[];
  ...
  constructor(deps: YaleHealthSciencesSummerProgramsScraperDeps = {}) {
    this.programSeeds = deps.programSeeds || CURATED_YALE_HEALTH_SCIENCES_SUMMER_PROGRAM_SEEDS;
    this.directoryUrls = deps.directoryUrls || YALE_HEALTH_SCIENCES_DIRECTORY_SEED_URLS;
  ...
  async run(ctx: ScraperContext): Promise<ScraperResult> {
    ...
    for (const directoryUrl of this.directoryUrls) {
      try {
        ...
        for (const url of extractProgramUrlsFromDirectory(html, directoryUrl)) {
          discoveredUrls.add(url);
        }
      } catch (error) {
        ctx.log('Skipping Yale health-sciences listing page after fetch/parse failure', {
          url: directoryUrl,
    ...
    const seededUrls = this.programSeeds
      ...
      .filter((url) => !isExcludedAlreadyCoveredUrl(url));
    const programUrls = Array.from(new Set([...seededUrls, ...discoveredUrls]))
      .filter(isYaleOwnedUrl)
      .filter((url) => !isExcludedAlreadyCoveredUrl(url))
      .slice(0, MAX_DISCOVERED_PROGRAM_PAGES);

    const candidatesByKey = new Map<string, HealthSciencesProgramCandidate>();
    const failedUrls: string[] = [];
    let fetchSuccesses = 0;

    for (const url of programUrls) {
      let parsed: HealthSciencesProgramCandidate | undefined;
      ...
      for (let attempt = 0; attempt < 3; attempt += 1) {
        try {
          ...
          fetched = true;
          parsed = parseHealthSciencesProgramPage(
            html,
            url,
      ...
      if (!fetched) {
        failedUrls.push(url);
        ctx.log('Skipping Yale health-sciences program page after fetch failure', {
          url,
    ...
    if (fetchSuccesses === 0 && failedUrls.length > 0) {
      throw new Error(
        `No Yale health-sciences summer program pages could be fetched; failed URLs: ${failedUrls.join(', ')}`,
      );
    }
    ...
    if (failedUrls.length > 0) {
      noteParts.push(
        `Skipped ${failedUrls.length} health-sciences summer program page(s) after fetch failure.`,
      );
    }
    ...
    return {
      ...
      metrics: {
        healthSciencesSummerPrograms: {
          ...
          directoryDiscovered: discoveredUrls.size,
