...
describe('synthesizeCoverageDescription', () => {
  ...
  /**
   * The demonstration #3201's arm removal owes. That arm used to catch a keyword-soup
   * summary; the body-versus-corpus question is now asked only at this module's own
   * `COVERAGE_MIN_OVERLAP`, so a summary built from a MeSH keyword run has to still be
   * refused here, or the contract fix traded one failure for another.
   */
