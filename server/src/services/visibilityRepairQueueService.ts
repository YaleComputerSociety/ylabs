...
interface RepairDeps {
  ...
  ensureNarrativeEvidenceObservation?: (input: {
    researchEntityId: string;
...
async function attemptResearchRepair(
  ...
  if (mode === 'apply' && Object.keys(patch).length > 0) {
    if (deps.ensureNarrativeEvidenceObservation) {
      for (const field of ['fullDescription', 'shortDescription'] as const) {
        const value = textValue(patch[field]);
        if (!value) continue;
        const provenance = await deps.ensureNarrativeEvidenceObservation({
          researchEntityId: plan.recordId,
          field,
          value,
          sourceUrl: repairSource,
        });
        if (provenance) patch[`fieldProvenance.${field}`] = provenance;
      }
    }
...
const defaultRepairDeps: RepairDeps = {
  ...
  async ensureNarrativeEvidenceObservation({ researchEntityId, field, value, sourceUrl }) {
    const entityId = toVisibilityRepairObjectId(researchEntityId);
    if (!entityId || !sourceUrl || !value) return null;
    ...
    if (!observation) {
      ...
      observation = (
        await Observation.create({
          ...
          confidence: 0.8,
          observedAt: new Date(),
          observationFingerprint: `visibility-repair:narrative:${researchEntityId}:${field}:${researchScopeEvidenceValueHash(value)}`,
        })
      ).toObject();
    }

    const observationId = idValue(observation._id);
    const sourceId = idValue(observation.sourceId);
    if (!observationId || !sourceId) return null;
    return {
      sourceId,
      observationId,
      sourceName: textValue(observation.sourceName),
