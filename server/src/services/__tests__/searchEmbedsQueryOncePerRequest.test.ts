...
describe('one student search embeds its query once (#3149)', () => {
  beforeEach(() => {
    ...
    mocks.embedSearchQuery.mockReset();
    mocks.embedSearchQuery.mockResolvedValue(QUERY_VECTOR);
  ...
  it('falls back to letting Meilisearch embed when no vector is available', async () => {
    mocks.embedSearchQuery.mockResolvedValue(null);
